 ...
 * @description register a new user, expacts username, email and password
 * @access Public
 */
async function registerUserController(req, res) {
  ...
  const isUserAleradyExist = await userModel.findOne({
    $or: [{ username }, { email }],
  });
...
async function logoutUserController(req, res) {
  ...
  if (token) {
    await tokenBlaklistModel.create({ token });
  }
